# -*- coding: utf-8 -*-
"""Sweep: decrease vector, increase graph, track relevant papers by channel."""
...
# Sweep configs: (VEC, GRAPH_K)
# Start high vector / low graph, shift towards low vector / high graph
CONFIGS = [
    (100,  5),
    (80,  10),
    (60,  15),
...
# Per-config cache (cleared between configs)
_bfs_cache = {}
...
# ---- Pre-cache vector results for max VEC to avoid redundant computation ----
print("Pre-caching vector results for all queries...")
vec_cache = {}
...
for vec_n, graph_k in CONFIGS:
    print(f"\n{'='*60}")
    print(f"CONFIG: VEC={vec_n}, GRAPH_K={graph_k}")
    print(f"{'='*60}")
    _bfs_cache.clear()
...
df.to_csv("channel_sweep_results.csv", index=False)
print("\nSaved channel_sweep_results.csv")
...
fig.suptitle("Vector vs Graph Sweep: Decreasing Vector, Increasing Graph", fontsize=14, fontweight="bold")
...
# Panel 2: Relevant papers by source (stacked bar)
ax = axes[0, 1]
...
# Panel 3: Pool size by source (stacked bar)
ax = axes[1, 0]
bottom = np.zeros(len(sweep_results))
for src, color in zip(sources, colors):
    ...
    ax.bar(x, vals, bottom=bottom, label=src, color=color)
    bottom += np.array(vals)
ax.set_ylabel("Total Pool Size")
ax.set_title("Pool Composition by Source")
ax.set_xticks(x)
ax.set_xticklabels(labels, rotation=45, ha="right")
ax.legend(fontsize=8)

# Panel 4: Relevance rate by source (line plot)
ax = axes[1, 1]
for src, color in zip(sources, colors):
    rates = []
...
plt.tight_layout()
plt.savefig("channel_sweep_results.png", dpi=150)
print("Saved channel_sweep_results.png")
