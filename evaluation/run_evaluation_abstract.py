...
Runs all retrieval configurations with abstract-only vector search
(chunk_source == "abstract" filter in Milvus), so PDF chunk bias is eliminated.
Results saved to evaluation_results_abstract_*.csv.
"""
import csv, json, os, random, time, sys, functools
...
def do_vector_search(query, n_papers=5):
    """Retrieve exactly n_papers unique papers using abstract chunks only."""
    BATCH = 500
    query_vec = create_embedding([query])[0]
    seen = set()
    result = []
    offset = 0
    print(f"    [vector] fetching {n_papers} unique papers (abstract-only)...", flush=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*70)
print("Running retrieval experiments (abstract-only vector search)...")
print("="*70)
...
with open(ROOT / "results" / "evaluation_results_abstract_detail.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=detail_rows[0].keys())
    writer.writeheader()
    writer.writerows(detail_rows)
print(f"  evaluation_results_abstract_detail.csv: {len(detail_rows)} rows")
...
with open(ROOT / "results" / "evaluation_results_abstract_summary.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=summary_rows[0].keys())
    writer.writeheader()
    writer.writerows(summary_rows)
print(f"  evaluation_results_abstract_summary.csv: {len(summary_rows)} rows")

# Print final table
print("\n" + "="*70)
print("FINAL RESULTS SUMMARY (abstract-only vector search)")
print("="*70)
